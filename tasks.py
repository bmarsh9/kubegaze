...
openssl req -x509 -new -nodes -key ca.key -days 100000 -out ca.crt -addext 'subjectAltName = DNS:admission_ca' -subj '/CN=admission_ca'"""
...
subjectAltName=DNS:webhook.webhook.svc
"""
        )

    print("==> Generating private key and certificate")
#openssl req -new -key server.key -out server.csr -addext "subjectAltName = DNS:{}" -config server.conf
